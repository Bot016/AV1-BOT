...
function show(nome) {
    ...
    if (nome && nome.trim !== "") {
        ...
        window.location.href = "#name-order"
    } else {
...
function calculateTotal() {
    ...
    for (let a = 0; a < selectVetors.length; a++) {
        if (selectVetors[a].value != "Selecione o avião desejado") {
            cleanPrices()
            ...
            alert("Selecione algum avião para começar!");
            break;
        }
    }
}
...
function showPrice(total, selectCategory, aircraft, price) {
    ...
    for (let a = 0; a < selectCategory.length; a++) {
        ...
        if (categorias.includes(selectCategory[a])) {
            var aircraftShow = aircraft[a] + ": " + price[a].toLocaleString('en-US', { style: 'currency', currency: 'USD' });
            document.getElementById(selectCategory[a] + "Price").textContent = aircraftShow
            showAirplane.push(aircraftShow)
        }
    }
...
function send() {
    if (globalTotal != "") {
        var message = `Olá, meu nome é ${nomeGlobal} e gostaria de fazer uma encomenda:\n`;
        for (let a = 0; a < showAirplane.length; a++) {
            message += showAirplane[a] + "\n"
        }
        message += `Total: ${globalTotal}\nData da encomenda: ${getDate()}`
        var encodedMessage = encodeURIComponent(message);
        var numeroTelefone = "5541992216924";
        var linkWhatsApp = "https://wa.me/" + numeroTelefone + "?text=" + encodedMessage
